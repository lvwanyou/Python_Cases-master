...
missile = pygame.image.load('I:/项目/python/模拟导弹自动追踪算法/rect1.png').convert_alpha()
bomb = pygame.image.load('I:/项目/python/模拟导弹自动追踪算法/bomb.png').convert_alpha()
...
while True:
    ...
    if distance > 1:
        ...
        screen.blit(bomb, (x-width/2, y-2*height/3))  # 将旋转后爆炸的点位的坐标进行稍微的位移后进行显示。
